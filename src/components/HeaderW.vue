...
<template>
  <header
    ...
    :class="[isScrolled ? 'bg-[#652bdf] shadow-lg py-4 z-50' : 'bg-transparent py-4 z-50']">
    <div
      ...
      <transition name="fade">
        <div v-if="isScrolled" class="flex items-center">
          <img src="/images/logo.png" alt="STUDYforME Logo" class="h-5" />
        </div>
      </transition>
      ...
      <nav
        ...
        <a
          ...
          :class="activeSection === menu.id ? 'font-bold' : 'font-normal'">
          {{ menu.name }}
        </a>
      </nav>
    </div>
